"""Sibling to test_detr.py for the FCAF3D-style dense head
(models/detector_fcaf3d.py). Reuses every rotation-agnostic helper from
test.py as-is (iou_matrix is volume-only/axis-aligned, so it doesn't care
whether the detector represents rotation as a quaternion or 6D) -- only the
per-batch forward()/decode() call signatures differ, since
DiverDetectorFCAF3D.forward() returns per-level predictions (not one query
set) and decode() needs level_batch_idx/batch_size to gather them back into
